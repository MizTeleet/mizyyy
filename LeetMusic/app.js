...
const detailRepeatBtn = document.getElementById('detailRepeatBtn');
...
function getCurrentTrackIndex() {
  if (!current) return -1;
  return tracks.findIndex((track) => track.url === current.url);
}

function updateRepeatUI() {
  const icon = repeatCurrentTrack ? '🔂' : '➡';
  const title = repeatCurrentTrack
    ? 'Повтор текущего трека включен'
    : 'После конца перейти к следующему треку';
  repeatBtn.textContent = icon;
  detailRepeatBtn.textContent = icon;
  repeatBtn.title = title;
  detailRepeatBtn.title = title;
  repeatBtn.classList.toggle('active', repeatCurrentTrack);
  detailRepeatBtn.classList.toggle('active', repeatCurrentTrack);
}
...
async function playTrackByIndex(index, openDetails = false) {
  if (index < 0 || index >= tracks.length) return;
  await playTrack(tracks[index], openDetails);
}

async function playAdjacentTrack(step, openDetails = false, stopAtEnd = false) {
  if (!tracks.length) return;
  const currentIndex = getCurrentTrackIndex();
  if (currentIndex === -1) {
    await playTrackByIndex(step >= 0 ? 0 : tracks.length - 1, openDetails);
    return;
  }

  const nextIndex = currentIndex + step;
  if (nextIndex < 0) {
    await playTrackByIndex(stopAtEnd ? 0 : tracks.length - 1, openDetails);
    return;
  }
  ...
  if (nextIndex >= tracks.length) {
    ...
    await playTrackByIndex(0, openDetails);
    return;
  }

  await playTrackByIndex(nextIndex, openDetails);
}
...
[prevBtn, detailPrevBtn].forEach((btn) =>
...
[nextBtn, detailNextBtn].forEach((btn) =>
  btn.addEventListener('click', async () => {
    await playAdjacentTrack(1);
  })
);

[repeatBtn, detailRepeatBtn].forEach((btn) => btn.addEventListener('click', toggleRepeatMode));
...
audio.addEventListener('ended', async () => {
  ...
  await playAdjacentTrack(1, false, true);
  updateTransportUI();
});
